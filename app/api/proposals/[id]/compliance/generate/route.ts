...
export async function POST(
  ...
  try {
    ...
    if (checkError) {
      console.error('[generate] Check error:', checkError)
      return NextResponse.json({ error: checkError.message }, { status: 500 })
    }

    if (existingItems && existingItems.length > 0) {
      return NextResponse.json(
        { error: 'Matrix already generated. Use regenerate to replace.' },
        { status: 409 }
      )
    }
    ...
    if (reqError) {
      console.error('[generate] Requirements fetch error:', reqError)
    ...
    if (wbsElements.length > 0 && itemsToInsert.length > 0) {
      console.log('[generate] Suggesting WBS links...')
      try {
        ...
        if (wbsLinkText && wbsLinkResponse.stop_reason !== 'max_tokens') {
          ...
          if (jsonStart !== -1 && jsonEnd !== -1) {
            ...
            console.log('[generate] WBS links suggested for', wbsLinks.length, 'requirements')
            wbsLinksGenerated = true
          }
        }
      } catch (wbsLinkError) {
        console.warn('[generate] WBS link suggestion failed:', wbsLinkError)
    ...
    console.error('Generate compliance error:', error)
    ...
    return NextResponse.json(
      { error: `Failed to generate matrix: ${errorMessage}` },
      { status: 500 }
    )
  }
}
